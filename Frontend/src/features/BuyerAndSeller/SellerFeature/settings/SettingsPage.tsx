...
import { ScreenShell } from '../../BuyerFeature/shared/ScreenShell';
...
const navigableItems: Record<string, string> = {
  'Organisation Profile': '/seller/onboarding/organisation',
  'Contact Management': '/seller/contacts?from=settings',
  // 'Activity Log': '/seller/audits', // Add if audits exist for seller
};

const comingSoonItems = new Set(['Preferred Categories', 'Preferred Locations', 'Quality Preferences', 'Users & Access', 'Data & Privacy', 'Activity Log']);

const groups = [
  { title: 'Profile & Organisation', items: [['Organisation Profile', Building2], ['Contact Management', Users]] },
  { title: 'Selling Preferences', items: [['Preferred Categories', Tags], ['Preferred Locations', MapPin], ['Quality Preferences', Shield]] },
          ...
          <Card className="p-5" key={g.title}>
            ...
            <div className="mt-4 space-y-2">
              {g.items.map(([label, Icon]) => {
                const to = navigableItems[label as string];
                const disabled = comingSoonItems.has(label as string);
                return (
                  <button
                    key={label as string}
                    disabled={disabled}
                  ...
                  >
                    <span className="flex items-center gap-3"><span className="grid h-8 w-8 place-items-center rounded-lg bg-slate-50 text-brand-600"><Icon className="h-4 w-4" /></span>{label as string}</span>
