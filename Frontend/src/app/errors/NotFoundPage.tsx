...
import page2 from "../../assets/P2jpeg.jpg"
...
const PARTNER_SLIDES = [
    ...
    },
    {
        ...
        direction: 'left',
        items: [
            { name: 'Adhiyamaan College of Engineering', src: adhiyamaanLogo, role: 'Academia Partner' },
            { name: 'The Rameshwaram Cafe', src: rameshwaramLogo, role: 'Hospitality Partner' },
            { name: 'Arha Multi Speciality Hospital', src: arhaLogo, role: 'Medical Partner' },
...
export function NotFoundPage(): React.JSX.Element {
    ...
    const [activePartnerSlide, setActivePartnerSlide] = useState<'all' | 'sponsors' | 'supported' | 'associations' | 'partners'>('all');
    const [activeBrochureTab, setActiveBrochureTab] = useState<BrochurePage>('p0');
    ...
    const [selectedStall, setSelectedStall] = useState<SelectedStall>({ id: 'A1', type: 'Platinum', rate: 63000 });
    ...
    const [successData, setSuccessData] = useState<SuccessData>({ id: 'LUB-TN-8911', stall: 'A1', company: 'Example' });
    const [showPromoPopup, setShowPromoPopup] = useState(false);
    const [stallSizes, setStallSizes] = useState<StallSizeOption[]>([]);
    const [allAvailableStalls, setAllAvailableStalls] = useState<AvailableStallSizeGroup[]>([]);
    ...
    useEffect(() => {
        ...
        fontLink.href = 'https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=Cinzel:wght@600;700;800&display=swap';
    ...
    const totalAvailableStalls = allAvailableStalls.reduce(
        (sum, group) =>
            sum +
            group.availableStalls.filter(s => !s.isSponsor).length,
        0
    );
    ...
    const selectStall = useCallback((id: string, type: StallType, rate: number) => {
    ...
    const simulateMatchmaking = useCallback(() => {
        ...
        if (matchProduct === 'cnc') {
            data = [
                { buyer: 'Ashok Leyland Sourcing Division', fit: '98% Fit', requirement: 'Transmission housings & gear blanks' },
                { buyer: 'Ather Energy Hosur Assembly', fit: '92% Fit', requirement: 'Chassis brackets & mounting panels' },
            ];
        } else if (matchProduct === 'pcb') {
            data = [
                { buyer: 'TVS Motor EV Division', fit: '95% Fit', requirement: 'Power controller boards & circuit setups' },
                { buyer: 'BEL Aerospace Sourcing Unit', fit: '90% Fit', requirement: 'Interconnect looms & wiring harnesses' },
            ];
        } else if (matchProduct === 'textiles') {
            data = [
                { buyer: 'Co-optex State Export Unit', fit: '99% Fit', requirement: 'Organic weaves & technical safety uniforms' },
            ];
        }
    ...
    const handleModalBackdropClick = useCallback((event: MouseEvent<HTMLDivElement>) => {
    ...
    const getBrochureTabClass = (page: BrochurePage): string => {
        ...
        return `brochure-tab flex-1 min-w-[150px] px-6 py-5 text-center font-extrabold text-sm uppercase tracking-wider transition focus:outline-none border-b-2 ${isActive ? 'border-lub-blue text-lub-blue' : 'border-transparent text-slate-400 hover:text-slate-700'
            }`;
    };
        ...
        <>
            ...
            <div className="enterprise-bg text-slate-800 font-sans selection:bg-lub-blue selection:text-white overflow-x-hidden">
                ...
                <nav className="fixed top-0 left-0 w-full z-50 bg-white/98 backdrop-blur-xl border-b border-slate-200/80 shadow-nav">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex items-center justify-between gap-4 h-20 sm:h-24">
                            ...
                            <div className="flex items-center gap-3 shrink-0">
                                <a href="#" className="flex shrink-0 items-center justify-center p-1.5 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition">
                                    <img src={ASSETS.lubLogo} alt="Laghu Udyog Bharati Official Logo" className="h-11 sm:h-14 w-auto object-contain block" />
                                </a>
                                <a href="#" className="flex shrink-0 items-center justify-center p-1.5 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition">
                                    <img src={BRAND.msmeLogoUrl} alt="MSME Sangamam Connect" className="h-11 sm:h-14 w-auto object-contain block" />
                                </a>
                            </div>

                            {/* Desktop Nav */}
                            <div className="hidden xl:flex items-center gap-8 shrink-0">
                                <a href="#about" onClick={(e) => handleSmoothScroll(e, "#about")} className="text-slate-700 hover:text-lub-blue transition duration-200 text-sm font-bold uppercase tracking-wider flex items-center gap-2 whitespace-nowrap"><span className="w-2 h-2 rounded-full bg-lub-orange shrink-0"></span> About</a>
                                <a href="#sectors" onClick={(e) => handleSmoothScroll(e, "#sectors")} className="text-slate-700 hover:text-lub-blue transition duration-200 text-sm font-bold uppercase tracking-wider flex items-center gap-2 whitespace-nowrap"><span className="w-2 h-2 rounded-full bg-lub-blue shrink-0"></span> Focus Sectors</a>
                                <a href="#heritage" onClick={(e) => handleSmoothScroll(e, "#heritage")} className="text-slate-700 hover:text-lub-blue transition duration-200 text-sm font-bold uppercase tracking-wider flex items-center gap-2 whitespace-nowrap"><span className="w-2 h-2 rounded-full bg-lub-green shrink-0"></span> Heritage Hub</a>
                                <a href="#supported-sponsored" onClick={(e) => handleSmoothScroll(e, "#supported-sponsored")} className="text-slate-700 hover:text-lub-blue transition duration-200 text-sm font-bold uppercase tracking-wider flex items-center gap-2 whitespace-nowrap"><span className="w-2 h-2 rounded-full bg-lub-orange shrink-0"></span> Supported &amp; Sponsored By</a>
                                <a href="#brochure" onClick={(e) => {
                                    handleSmoothScroll(e, "#brochure")
                                    switchBrochureTab("p2")
                                }} className="text-slate-700 hover:text-lub-blue transition duration-200 text-sm font-bold uppercase tracking-wider flex items-center gap-2 whitespace-nowrap"><span className="w-2 h-2 rounded-full bg-slate-400 shrink-0"></span> Stall Map</a>
                            </div>

                            {/* CTA */}
                            <div className="hidden sm:flex items-center gap-3 shrink-0">
                                <Link to="/stall-booking" className="px-5 py-3 rounded-lg text-sm font-extrabold uppercase tracking-wider bg-gradient-to-r from-lub-blue to-lub-enterpriseMid text-white hover:shadow-glow-blue transition duration-300 transform hover:-translate-y-0.5 shadow-md whitespace-nowrap">
                                    <i className="fa-solid fa-hotel mr-2"></i>Book Stall
                                </Link>
                                <Link to="/login" className="px-5 py-3 rounded-lg text-sm font-extrabold uppercase tracking-wider border-2 border-lub-blue text-lub-blue hover:bg-lub-blue hover:text-white transition duration-300 transform hover:-translate-y-0.5 whitespace-nowrap">
                            ...
                            {/* Mobile Menu Button */}
                            <button type="button" onClick={() => setMobileMenuOpen((open) => !open)} className="xl:hidden shrink-0 text-slate-700 hover:text-lub-blue focus:outline-none p-2 rounded-xl bg-slate-100 border border-slate-200">
                    ...
                    {/* Mobile Drawer */}
                    <div className={`${mobileMenuOpen ? '' : 'hidden '}xl:hidden bg-white border-b border-slate-200 py-8 px-8 space-y-5 shadow-inner`}>
                        <a href="#about" onClick={(e) => handleSmoothScroll(e, "#about")} className="block text-slate-800 hover:text-lub-blue font-extrabold text-lg py-2">About Expo</a>
                        <a href="#sectors" onClick={(e) => handleSmoothScroll(e, "#sectors")} className="block text-slate-800 hover:text-lub-blue font-extrabold text-lg py-2">Focus Sectors</a>
                        <a href="#heritage" onClick={(e) => handleSmoothScroll(e, "#heritage")} className="block text-slate-800 hover:text-lub-blue font-extrabold text-lg py-2">Heritage Hub</a>
                        <a href="#supported-sponsored" onClick={(e) => handleSmoothScroll(e, "#supported-sponsored")} className="block text-slate-800 hover:text-lub-blue font-extrabold text-lg py-2">Supported &amp; Sponsored By</a>
                        <a href="#brochure" onClick={(e) => {
                            handleSmoothScroll(e, "#floorplan")
                            switchBrochureTab("p2")
                        }} className="block text-slate-800 hover:text-lub-blue font-extrabold text-lg py-2">Stall Map</a>
                        <Link to="/stall-booking" className="block text-center py-4 bg-gradient-to-r from-lub-blue to-lub-enterpriseMid text-white rounded-xl font-extrabold text-lg shadow-md mt-4">
                            <i className="fa-solid fa-hotel mr-2"></i>Book a Stall Now
                        </Link>
                        <Link to="/login" className="block text-center py-4 border-2 border-lub-blue text-lub-blue rounded-xl font-extrabold text-lg mt-4">
                            <i className="fa-solid fa-right-to-bracket mr-2"></i>Login
                        </Link>
                                    ...
                                    <div className="p-6 text-center flex-1 flex flex-col justify-between">
                                        <div>
                                            ...
                                            <p className="mt-3 text-xs sm:text-sm text-slate-600 leading-relaxed font-medium">
                                                Hon&apos;ble Minister of State for MSME and Labour &amp; Employment, Government of India
                                            </p>
                                        </div>
                                ...
                                <div className="relative flex-1 flex flex-col justify-between overflow-hidden rounded-[2rem] bg-white border border-slate-200 shadow-card hover:shadow-card-hover transition duration-300">
                                    ...
                                    <div className="p-6 text-center flex-1 flex flex-col justify-between">
                                        <div>
                                            ...
                                            <p className="mt-3 text-xs sm:text-sm text-slate-600 leading-relaxed font-medium">
                                                Hon&apos;ble Minister of State for Information &amp; Broadcasting and Parliamentary Affairs Government of India
                                            </p>
                                        </div>
                        ...
                        <div className="mt-14 lg:mt-16 text-center max-w-4xl mx-auto flex flex-col items-center">
                            ...
                            <div className="mt-6 space-y-4">
                                ...
                                <p className="text-base sm:text-lg text-slate-600 leading-relaxed font-medium max-w-3xl mx-auto">
                                    ...
                                    . Meet buyers, industry leaders, government organizations,
                                    and manufacturing partners under one platform.
                                </p>
                            </div>
                            ...
                            {/* Primary Action Buttons (Above - Centered) */}
                            ...
                            {/* B2B Marketplace Action Buttons (Bottom - Centered Separately) */}
                            <div className="mt-3.5 sm:mt-4 flex flex-wrap items-center justify-center gap-3 sm:gap-4">
                                <button
                        ...
                        <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                            <div className="enterprise-card rounded-2xl p-6">
                                ...
                                <p className="mt-2 text-sm text-slate-600 font-medium leading-relaxed">
                                    Direct business meetings with industrial and institutional buyers.
                                </p>
                            </div>
                            ...
                            <div className="enterprise-card rounded-2xl p-6">
                                ...
                                <p className="mt-2 text-sm text-slate-600 font-medium leading-relaxed">
                                    Connect with defence, aerospace, automotive and PSU procurement teams.
                                </p>
                            </div>
                            ...
                            <div className="enterprise-card rounded-2xl p-6">
                                ...
                                <p className="mt-2 text-sm text-slate-600 font-medium leading-relaxed">
                                    Present products, manufacturing capability and innovation to buyers.
                                </p>
                            </div>
                            ...
                            <div className="enterprise-card rounded-2xl p-6">
                                ...
                                <p className="mt-2 text-sm text-slate-600 font-medium leading-relaxed">
                                    A dedicated platform for women-led MSMEs and product innovators.
                                </p>
                            </div>
                        </div>

                        {/* Sponsor & Key Partners 4 Slides with 4 Headings and Moving Cards */}
                        <div id="supported-sponsored" className="mt-16 pt-10 border-t border-slate-200 scroll-mt-28">
                            <div className="text-center max-w-3xl mx-auto mb-8">
                                ...
                                <p className="text-xs sm:text-sm text-slate-500 mt-2 font-medium">
                                    Proudly backed by prestigious sponsors, governmental bodies, apex industry associations, and ecosystem partners.
                                </p>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-10">
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer ${activePartnerSlide === 'all'
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer ${activePartnerSlide === 'supported'
                                        ? 'bg-lub-blue text-white shadow-md scale-105'
                                        : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer ${activePartnerSlide === 'associations'
                                        ? 'bg-lub-green text-white shadow-md scale-105'
                                        : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
                                        }`}
                                >
                                    <span className="opacity-70 mr-1">2.</span> Supporting Industry Associations
                                </button>
                                <button
                                    type="button"
                                    onClick={() => setActivePartnerSlide('sponsors')}
                                    className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer ${activePartnerSlide === 'sponsors'
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer ${activePartnerSlide === 'partners'
                            ...
                            <div className="space-y-10">
                                {PARTNER_SLIDES.filter(
                                    (slide) => activePartnerSlide === 'all' || activePartnerSlide === slide.id
                                ).map((slide) => {
                                    const isSupported = slide.id === 'supported';
                                    const marqueeItems = isSupported ? [] : getRepeatedMarqueeItems(slide.items);
                                    return (
                                        <div
                                            key={slide.id}
                                        ...
                                        >
                                            ...
                                            {isSupported ? (
                                                ...
                                                <div className="relative w-full overflow-hidden marquee-container py-2">
                                                    ...
                                                    {/* Marquee Track */}
                                                    <div
                                                        className={`flex gap-6 ${slide.direction === 'left'
                                                            ? 'marquee-track-left'
                                                            : 'marquee-track-right'
                                                            }`}
                                                    >
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
                        ...
                        <div className="lg:col-span-7 space-y-10 text-center lg:text-left">
                            ...
                            <div className="space-y-5">
                                <span className="text-sm uppercase font-extrabold tracking-widest text-lub-orange block">TAMIL NADU STATE B2B CONCLAVE 2026</span>
                                <h1 className="text-5xl sm:text-6xl md:text-7xl font-extrabold tracking-tight leading-tight text-slate-900">
                                    Where Ancient <span className="text-lub-gold font-heritage">Heritage</span><br />
                                    Meets <span className="text-transparent bg-clip-text bg-gradient-to-r from-lub-blue via-lub-orange to-lub-green">Industrial Future</span>
                                </h1>
                            </div>

                            <p className="text-xl text-slate-600 max-w-2xl mx-auto lg:mx-0 font-medium leading-relaxed">
                                Welcome to <strong className="text-slate-900 font-bold">MSME Sangamam Connect 2026</strong>. Hosted at <span className="text-lub-orange font-bold underline decoration-lub-green decoration-2 underline-offset-4">Hosur</span>, the industrial powerhouse gateway of South India, connecting local MSMEs directly to PSU procurement cells and global defense supply ecosystems.
                            </p>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                                <div className="flex items-center space-x-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-card">
                                    ...
                                    <div className="text-left">
                                        <div className="text-xs text-slate-500 font-bold tracking-wider uppercase">EXPO DATES</div>
                                        <div className="text-lg font-extrabold text-slate-900">18 & 19 September 2026</div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center space-x-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-card">
                                    ...
                                    <div className="text-left">
                                        <div className="text-xs text-slate-500 font-bold tracking-wider uppercase">PREMIUM VENUE</div>
                                        <div className="text-lg font-extrabold text-slate-900">Hotel Hills, Hosur, TN</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-5 pt-2">
                                ...
                                <a href="#b2b-matcher" onClick={(e) => handleSmoothScroll(e, "#b2b-matcher")} className="w-full sm:w-auto px-10 py-5 bg-white border-2 border-slate-200 rounded-xl font-extrabold text-lub-blue hover:bg-lub-blue/5 hover:border-lub-blue/40 transition duration-300 text-center flex items-center justify-center gap-2 text-lg">
                            ...
                            <div className="pt-6 border-t border-slate-200">
                                <div className="text-xs tracking-widest text-slate-500 uppercase font-extrabold mb-4 text-center lg:text-left">CONGRESS COUNTDOWN</div>
                                <div className="grid grid-cols-4 gap-4 max-w-md mx-auto lg:mx-0">
                                    <div className="bg-white p-4 rounded-xl border border-slate-200 text-center shadow-card">
                                        <div className="text-3xl font-extrabold text-lub-orange">{countdown.days}</div>
                                        <div className="text-[10px] text-slate-500 uppercase font-extrabold mt-1">Days</div>
                                    </div>
                                    <div className="bg-white p-4 rounded-xl border border-slate-200 text-center shadow-card">
                                        <div className="text-3xl font-extrabold text-lub-blue">{countdown.hours}</div>
                                        <div className="text-[10px] text-slate-500 uppercase font-extrabold mt-1">Hours</div>
                                    </div>
                                    <div className="bg-white p-4 rounded-xl border border-slate-200 text-center shadow-card">
                                        <div className="text-3xl font-extrabold text-lub-green">{countdown.minutes}</div>
                                        <div className="text-[10px] text-slate-500 uppercase font-extrabold mt-1">Mins</div>
                                    </div>
                                    <div className="bg-white p-4 rounded-xl border border-slate-200 text-center shadow-card">
                                        <div className="text-3xl font-extrabold text-slate-800">{countdown.seconds}</div>
                                        <div className="text-[10px] text-slate-500 uppercase font-extrabold mt-1">Secs</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-5 relative flex justify-center items-center">
                            <div className="relative w-full max-w-lg bg-white p-6 rounded-3xl shadow-card-hover border border-slate-200 overflow-hidden transform hover:scale-[1.01] transition duration-500">
                                ...
                                <img src={ASSETS.msmeSangamam} alt="Official LUB MSME Sangamam" className="w-full h-auto object-contain rounded-xl" />
                                ...
                                <div className="mt-5 p-5 bg-blue-50 rounded-xl border border-blue-100 text-slate-800">
                                    <div className="flex items-center justify-between text-sm font-bold mb-3">
                                        <span className="text-lub-blue">Exhibitor Space Allocation</span>
                                        <span className="text-lub-green flex items-center gap-1"><i className="fa-solid fa-shield-check"></i> Certified</span>
                                    </div>
                                    <p className="text-sm text-slate-600 leading-relaxed font-medium">
                                        Promoted under Laghu Udyog Bharati, reinforcing domestic enterprise capabilities, localization setups, and high engineering compliance globally.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                {/* ===== ABOUT SECTION ===== */}
                <section id="about" className="py-28 relative bg-white border-t border-slate-100">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
                            <div className="lg:col-span-5 relative">
                                <div className="relative rounded-3xl overflow-hidden border-2 border-lub-orange/20 shadow-glow-orange bg-white p-4">
                                    <img src={ASSETS.msmeSangamam} alt="MSME Sangamam Connect Official Invitation" className="w-full h-auto object-cover rounded-2xl shadow-xl" />
                                </div>
                            </div>

                            <div className="lg:col-span-7 space-y-8">
                                <span className="text-lub-orange font-extrabold uppercase tracking-widest text-sm">INDUSTRIAL EMPOWERMENT</span>
                                <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-900 leading-tight">Empowering Indian Small &amp; Micro Industries</h2>
                                <p className="text-lg text-slate-600 leading-relaxed font-medium">
                                    Laghu Udyog Bharati stands as India's premier organization committed strictly to assisting micro and small manufacturing units. This major physical assembly at <strong className="text-slate-900 font-bold">Hotel Hills, Hosur</strong> directly serves as a B2B procurement match-point.
                                </p>
                                ...
                                <div className="p-6 rounded-2xl bg-gradient-to-r from-lub-orange/5 to-lub-blue/5 border border-slate-200 space-y-4">
                                    <h4 className="text-base font-bold text-lub-gold font-heritage flex items-center gap-2">
                                        <i className="fa-solid fa-dharmachakra text-xl"></i> Heritage of Precision Craftsmanship
                                    </h4>
                                    <p className="text-sm text-slate-600 leading-relaxed font-medium">
                                        Tamil Nadu's industry didn't start in the 20th century. Centuries ago, Chola merchant networks exported premium steel and rich fabrics across global maritime routes. This legacy lives on today through advanced CNC tooling, smart EV hubs, and technical defense corridors.
                                    </p>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                                    <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-card space-y-3 hover:shadow-card-hover transition">
                                        ...
                                        <h4 className="text-slate-900 font-extrabold text-lg">5,500+ Trade Visitors</h4>
                                        <p className="text-sm text-slate-600 font-medium">Engineers, industrial buyers, and defense procurement decision makers.</p>
                                    </div>
                                    <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-card space-y-3 hover:shadow-card-hover transition">
                                        <i className="fa-solid fa-link text-lub-blue text-2xl"></i>
                                        <h4 className="text-slate-900 font-extrabold text-lg">Direct Vendor Linkages</h4>
                                        <p className="text-sm text-slate-600 font-medium">Structured vendor enrollment paths for major tier-1 automotive companies.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <section id="sectors" className="py-28 relative bg-lub-enterpriseBg">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="text-center max-w-3xl mx-auto mb-20">
                            <span className="text-lub-orange font-extrabold uppercase tracking-widest text-sm">SECTORS OF PRIDE</span>
                            <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-900 mt-3 leading-tight">Tamil Nadu's Global Engineering Hegemony</h2>
                            <p className="text-lg text-slate-600 mt-5 font-medium">Connecting highly specialized small-scale manufacturers directly to multi-modal regional logistics grids.</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                            ...
                            <div className="md:col-span-8 enterprise-card rounded-3xl p-10 flex flex-col justify-between relative overflow-hidden group">
                                ...
                                <div className="flex justify-between items-start">
                                    ...
                                    <span className="text-xs text-lub-orange tracking-widest uppercase font-extrabold px-3 py-1.5 bg-lub-orange/8 rounded-full border border-lub-orange/15">Manufacturing Core</span>
                                </div>
                                <div className="mt-10 space-y-4">
                                    <h3 className="text-3xl font-extrabold text-slate-900">Automotive &amp; Electric Vehicle Machining</h3>
                                    <p className="text-lg text-slate-600 leading-relaxed font-medium">
                                        Hosur leads South Asia's EV ecosystem. This category binds regional component factories with global heavy OEMs, securing precision components.
                                    </p>
                                </div>
                                <div className="mt-8 pt-6 border-t border-slate-100 flex flex-wrap items-center gap-8 text-sm text-slate-500 font-semibold">
                                    <span><strong className="text-lub-orange">1,500+</strong> Local MSMEs</span>
                                    <span><strong className="text-lub-orange">ZED</strong> Certified Standards</span>
                                </div>
                            </div>
                            ...
                            <div className="md:col-span-4 enterprise-card rounded-3xl p-10 flex flex-col justify-between relative overflow-hidden group">
                                ...
                                <div className="flex justify-between items-start">
                                    ...
                                    <span className="text-xs text-lub-blue tracking-widest uppercase font-extrabold px-3 py-1.5 bg-lub-blue/8 rounded-full border border-lub-blue/15">Defense Corridor</span>
                                </div>
                                <div className="mt-10 space-y-4">
                                    <h3 className="text-2xl font-extrabold text-slate-900">Aerospace &amp; CNC Precision</h3>
                                    <p className="text-base text-slate-600 leading-relaxed font-medium">
                                        Active integration into Tamil Nadu's strategically designated aerospace corridors, onboarding high-grade military suppliers.
                                    </p>
                                </div>
                            ...
                            <div className="md:col-span-4 enterprise-card rounded-3xl p-10 flex flex-col justify-between relative overflow-hidden group">
                                ...
                                <div className="flex justify-between items-start">
                                    ...
                                    <span className="text-xs text-lub-gold tracking-widest uppercase font-extrabold px-3 py-1.5 bg-lub-gold/8 rounded-full border border-lub-gold/15">Legacy Weaves</span>
                                </div>
                                <div className="mt-10 space-y-4">
                                    <h3 className="text-2xl font-extrabold text-slate-900">Textiles &amp; Technical Fibers</h3>
                                    <p className="text-base text-slate-600 leading-relaxed font-medium">
                                        Preserving Tamil Nadu's iconic heritage weaving lines while pivoting into exports, showcasing highly skilled women-led micro businesses.
                                    </p>
                                </div>
                                <div className="mt-8 pt-6 border-t border-slate-100 text-sm text-slate-500 font-semibold">
                            ...
                            <div className="md:col-span-8 enterprise-card rounded-3xl p-10 flex flex-col justify-between relative overflow-hidden group">
                                ...
                                <div className="flex justify-between items-start">
                                    ...
                                    <span className="text-xs text-lub-green tracking-widest uppercase font-extrabold px-3 py-1.5 bg-lub-green/8 rounded-full border border-lub-green/15">Smart Panel</span>
                                </div>
                                <div className="mt-10 space-y-4">
                                    <h3 className="text-3xl font-extrabold text-slate-900">Automation, Smart Tooling &amp; Panels</h3>
                                    <p className="text-lg text-slate-600 leading-relaxed font-medium">
                                        Specialized machine manufacturers exhibiting smart power grid systems, automated PCB routers, and high-strength casting setups.
                                    </p>
                                </div>
                                <div className="mt-8 pt-6 border-t border-slate-100 flex flex-wrap items-center gap-8 text-sm text-slate-500 font-semibold">
                                    <span><strong className="text-lub-green">100%</strong> Local Manufacturing</span>
                ...
                <section id="heritage" className="py-28 relative bg-white">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="text-center max-w-3xl mx-auto mb-20">
                            <span className="text-lub-gold font-heritage font-extrabold uppercase tracking-widest text-sm">THE JOURNEY OF CRAFT</span>
                            <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-900 mt-3 leading-tight">Tamil Nadu's Industrial Legacy Node</h2>
                            <p className="text-lg text-slate-600 mt-5 font-medium">Tracing our transition from ancient international maritime trade channels to premier South-Asian multi-modal manufacturing hubs.</p>
                        </div>
                        ...
                        <div className="relative border-l-2 border-lub-orange/30 md:border-l-0 md:grid md:grid-cols-4 gap-8 pl-8 md:pl-0">
                            ...
                            <div className="relative enterprise-card p-8 rounded-2xl mb-10 md:mb-0">
                                <div className="text-sm font-extrabold text-lub-orange uppercase mb-2">Chola Merchant Guilds</div>
                                <h4 className="text-xl font-extrabold text-slate-900 mb-3">Maritime Wootz Trade</h4>
                                <p className="text-sm text-slate-600 leading-relaxed font-medium">
                                    Historical records trace advanced iron metallurgy and high-grade weaves exported across global oceans, forming the roots of regional merchant self-reliance.
                                </p>
                            </div>

                            <div className="relative enterprise-card p-8 rounded-2xl mb-10 md:mb-0">
                                <div className="text-sm font-extrabold text-lub-blue uppercase mb-2">Engineering Foundation</div>
                                <h4 className="text-xl font-extrabold text-slate-900 mb-3">Industrial Estates Evolve</h4>
                                <p className="text-sm text-slate-600 leading-relaxed font-medium">
                                    Setting up industrial centers post-independence built the robust infrastructure and highly skilled workforce that paved the way for automated tooling.
                                </p>
                            </div>

                            <div className="relative enterprise-card p-8 rounded-2xl mb-10 md:mb-0">
                                <div className="text-sm font-extrabold text-lub-green uppercase mb-2">Detroit of South Asia</div>
                                <h4 className="text-xl font-extrabold text-slate-900 mb-3">Hosur-Chennai Auto Hub</h4>
                                <p className="text-sm text-slate-600 leading-relaxed font-medium">
                                    Hosur transforms into a massive manufacturing core, housing Tier-1 auto majors, aerospace facilities, and advanced precision components.
                                </p>
                            </div>

                            <div className="relative bg-white p-8 rounded-2xl border-2 border-lub-orange/30 shadow-glow-orange mb-10 md:mb-0">
                                <div className="text-sm font-extrabold text-lub-orange uppercase mb-2">2026 &amp; Beyond</div>
                                <h4 className="text-xl font-extrabold text-slate-900 mb-3">The EV &amp; Defense Pivot</h4>
                                <p className="text-sm text-slate-800 leading-relaxed font-bold">
                                    Focusing on sustainable smart mobility and localization corridors, ensuring small manufacturers drive the next generation of industrial growth.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                {/* ===== BROCHURE SECTION ===== */}
                <section id="brochure" className="py-28 bg-lub-enterpriseBg relative border-t border-slate-100">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="text-center max-w-3xl
                         mx-auto mb-20">
                            <span className="text-lub-orange font-extrabold uppercase tracking-widest text-sm">EVENT DOCUMENT GALLERY</span>
                            <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-900 mt-3 leading-tight">Interactive Event Brochure</h2>
                            <p className="text-lg text-slate-600 mt-5 font-medium">Preview official flyer layers, layout diagrams, bank channels, and organizing coordinators directly.</p>
                        </div>

                        <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-card-hover">
                            <div className="flex flex-wrap border-b border-slate-200 bg-slate-50">
                                <button type="button" onClick={() => switchBrochureTab("p0")} className={getBrochureTabClass("p0")}>
                                    <i className="fa-solid fa-file-invoice mr-2"></i> Event Overview
                                </button>
                                <button type="button" onClick={() => switchBrochureTab("p2")} className={getBrochureTabClass("p2")}>
                                    <i className="fa-solid fa-table-cells mr-2"></i> Hall Layout
                                </button>
                                <button type="button" onClick={() => switchBrochureTab("p3")} className={getBrochureTabClass("p3")}>
                                    <i className="fa-solid fa-sitemap mr-2"></i> Stall Categories
                                </button>
                                <button type="button" onClick={() => switchBrochureTab("p4")} className={getBrochureTabClass("p4")}>
                                    <i className="fa-solid fa-money-check-dollar mr-2"></i> Official Payments
                                </button>
                            </div>
                            ...
                            <div className="p-6 sm:p-10 flex flex-col md:flex-row items-center gap-10">
                                <div className="w-full md:w-3/5 bg-slate-50 rounded-2xl overflow-hidden p-3 border border-slate-200 flex justify-center items-center">
                                    <div className="relative group cursor-pointer w-full bg-slate-50 min-h-[400px] flex justify-center items-center">
                                        <img src={brochureSrc.p0} alt="Official brochure banner page 1" className={`brochure-image w-full h-auto max-h-[600px] object-contain rounded-xl transition duration-300${activeBrochureTab === "p0" ? "" : " hidden"}`} />
                                        <img src={brochureSrc.p2} alt="Floorplan categories layout page 2" onError={() => handleBrochureError("p2")} className={`brochure-image w-full h-auto max-h-[600px] object-contain rounded-xl transition duration-300${activeBrochureTab === "p2" ? "" : " hidden"}`} />
                                        <img src={brochureSrc.p3} alt="Exhibition floor stall scheme page 3" onError={() => handleBrochureError("p3")} className={`brochure-image w-full h-auto max-h-[600px] object-contain rounded-xl transition duration-300${activeBrochureTab === "p3" ? "" : " hidden"}`} />
                                        <img src={brochureSrc.p4} alt="Organizers contacts and registration details page 4" onError={() => handleBrochureError("p4")} className={`brochure-image w-full h-auto max-h-[600px] object-contain rounded-xl transition duration-300${activeBrochureTab === "p4" ? "" : " hidden"}`} />
                                        ...
                                        <div className="absolute inset-0 bg-lub-blue/10 opacity-0 group-hover:opacity-100 flex items-center justify-center transition duration-200 rounded-xl">
                                            <span className="bg-white/95 backdrop-blur-md px-5 py-3 rounded-lg text-sm font-bold text-lub-blue border border-slate-200">
                                                <i className="fa-solid fa-expand mr-2"></i> Preserved Official Material
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div
                                    className={`brochure-meta w-full md:w-2/5 space-y-7${activeBrochureTab === "p0" ? "" : " hidden"
                                        }`}
                                >
                                    ...
                                    <p className="text-base text-slate-600 leading-relaxed font-medium">
                                        Join Tamil Nadu's premier B2B MSME Exhibition & Buyer–Seller Meet on
                                        <strong className="text-slate-900"> 18 &amp; 19 September 2026 </strong>
                                        at
                                        <strong className="text-slate-900"> Hotel Hills, Hosur</strong>.
                                        Connect with industry leaders, government organizations, institutional
                                        buyers, defence & aerospace companies, and MSMEs to explore new business
                                    ...
                                    <div className="border-t border-slate-100 pt-5 space-y-4">
                                        ...
                                        <div className="flex items-center gap-3 text-base text-slate-700 font-semibold">
                                            ...
                                            <span>18 &amp; 19 September 2026 • Hotel Hills, Hosur</span>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-3 text-base text-slate-700 font-semibold">
                                            ...
                                            <span>Exclusive Buyer–Seller Meetings & Business Networking</span>
                                        </div>

                                        <div className="flex items-center gap-3 text-base text-slate-700 font-semibold">
                                            <i className="fa-solid fa-industry text-lub-green text-lg"></i>
                                            <span>MSME Product Showcase & Manufacturing Excellence</span>
                                        </div>

                                        <div className="flex items-center gap-3 text-base text-slate-700 font-semibold">
                                            <i className="fa-solid fa-jet-fighter text-lub-green text-lg"></i>
                                            <span>Defence, Aerospace, Auto & PSU Buyer Participation</span>
                                        </div>

                                        <div className="flex items-center gap-3 text-base text-slate-700 font-semibold">
                                            <i className="fa-solid fa-globe text-lub-green text-lg"></i>
                                            <span>International Buyers & Global Business Opportunities</span>
                                        </div>
                                    </div>
                                ...
                                <div
                                    className={`brochure-meta w-full md:w-2/5 space-y-7${activeBrochureTab === "p2" ? "" : " hidden"
                                        }`}
                                >
                                    <div className="space-y-3">
                                    ...
                                    <p className="text-base text-slate-600 leading-relaxed font-medium">
                                        ...
                                        . The floor plan provides stall locations, sponsor zones, meeting
                                        rooms, registration counters, entry & exit points, and visitor
                                        movement to help exhibitors choose the best business location.
                                    </p>
                                    ...
                                    <div className="border-t border-slate-100 pt-5 space-y-4">
                                        ...
                                        <div className="flex items-center gap-3 text-base text-slate-700 font-semibold">
                                            ...
                                            <span>Clearly Marked Entry, Exit & Registration Areas</span>
                                        </div>
                                ...
                                <div className={`brochure-meta w-full md:w-2/5 space-y-7${activeBrochureTab === "p3" ? "" : " hidden"}`}>
                                    <div className="space-y-3">
                                        <span className="text-sm font-extrabold text-lub-blue uppercase tracking-widest">Document Layer 3</span>
                                        <h3 className="text-3xl font-extrabold text-slate-900">Stall Categories &amp; Pricing</h3>
                                    </div>
                                    <p className="text-base text-slate-600 leading-relaxed font-medium">
                                        Detailed layout structure highlighting regional categorizations. Complete pricing rates with customized fascia and booth fixtures provided.
                                    </p>
                                    <div className="space-y-3 bg-slate-50 p-6 rounded-xl border border-slate-200">
                                        <div className="flex justify-between text-xs text-slate-500 pb-2 border-b border-slate-100 font-extrabold uppercase">
                                            <span>CATEGORY</span><span>RATE (INR)</span>
                                        </div>
                                        <div className="flex justify-between text-base font-bold text-slate-900">
                                            <span>Silver Stall (2m x 2m)</span><span className="text-lub-orange">₹28,000</span>
                                        </div>
                                        <div className="flex justify-between text-base font-bold text-slate-900">
                                            <span>Gold Stall (2m x 3m)</span><span className="text-lub-orange">₹42,000</span>
                                        </div>
                                        <div className="flex justify-between text-base font-bold text-slate-900">
                                            <span>Platinum Stall (3m x 3m)</span><span className="text-lub-orange">₹63,000</span>
                                        </div>
                                    </div>
                                </div>

                                <div className={`brochure-meta w-full md:w-2/5 space-y-7${activeBrochureTab === "p4" ? "" : " hidden"}`}>
                                    <div className="space-y-3">
                                        <span className="text-sm font-extrabold text-lub-gold uppercase tracking-widest">Document Layer 4</span>
                                        <h3 className="text-3xl font-extrabold text-slate-900">Official Payments &amp; Escrow</h3>
                                    </div>
                                    <p className="text-base text-slate-600 leading-relaxed font-medium">
                                        Official financial channels managed through Laghu Udyog Bharati Canara Bank accounts. Real-time coordinator telephone listing is detailed below.
                                    </p>
                                    <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 space-y-3 text-sm font-semibold">
                                        <div className="flex justify-between"><span className="text-slate-500">Account Name:</span> <span className="text-slate-900">Laghu Udyog Bharati</span></div>
                                        <div className="flex justify-between"><span className="text-slate-500">Bank Name:</span> <span className="text-slate-900">Canara Bank</span></div>
                                        <div className="flex justify-between"><span className="text-slate-500">IFSC Code:</span> <span className="text-lub-orange font-mono">CNRB0000936</span></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {/* ===== BOOKING MODAL ===== */}
                <div onClick={handleModalBackdropClick} className={`fixed inset-0 z-50 overflow-y-auto bg-slate-900/70 backdrop-blur-md flex items-center justify-center p-4${modalOpen ? "" : " hidden"}`}>
                    <div className="bg-white rounded-3xl border border-slate-200 max-w-lg w-full overflow-hidden shadow-2xl">
                        <div className="bg-slate-50 px-8 py-5 border-b border-slate-200 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-base font-extrabold text-slate-900 uppercase tracking-wider">Secure Booth Escrow Hold</h3>
                            </div>
                            <button type="button" onClick={closeModal} className="text-slate-400 hover:text-slate-700 transition focus:outline-none">
                                <i className="fa-solid fa-xmark text-2xl"></i>
                            </button>
                        </div>

                        <div className="w-full bg-slate-100 h-1.5">
                            <div style={{ width: modalProgress }} className="bg-gradient-to-r from-lub-blue via-lub-orange to-lub-green h-1.5 transition-all duration-300"></div>
                        </div>

                        <div className={`p-8 space-y-6${bookingStep === 1 ? "" : " hidden"}`}>
                            <div className="bg-blue-50 p-5 rounded-xl border border-blue-100 space-y-2">
                                <div className="text-[10px] text-slate-500 uppercase tracking-widest font-extrabold">CHOSEN EXHIBITION GRID NODE</div>
                                <div className="flex items-center justify-between">
                                    <span className="text-2xl font-extrabold text-slate-900">{selectedStall.id}</span>
                                    <span className="text-sm font-extrabold uppercase px-3 py-1 bg-lub-orange/10 text-lub-orange rounded-lg border border-lub-orange/20">{selectedStall.type}</span>
                                </div>
                                <div className="flex items-center justify-between pt-3 border-t border-blue-100 text-sm">
                                    <span className="text-slate-600 font-semibold">Escrow Transfer Total:</span>
                                    <span className="font-extrabold text-lub-gold text-xl">{`₹${selectedStall.rate.toLocaleString("en-IN")}`}</span>
                                </div>
                            </div>

                            <form id="stall-booking-form" onSubmit={submitBooking} className="space-y-5">
                                <div className="space-y-2">
                                    <label className="text-xs text-slate-600 font-extrabold uppercase">Enterprise Registered Name *</label>
                                    <input type="text" required value={formCompany} onChange={(e) => setFormCompany(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-5 py-3 text-slate-900 focus:outline-none focus:border-lub-blue text-base font-medium transition" placeholder="e.g., Hosur Engineering Gears Ltd" />
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <div className="space-y-2">
                                        <label className="text-xs text-slate-600 font-extrabold uppercase">Coordinator Name *</label>
                                        <input type="text" required value={formName} onChange={(e) => setFormName(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-5 py-3 text-slate-900 focus:outline-none focus:border-lub-blue text-base font-medium transition" placeholder="Full Name" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-xs text-slate-600 font-extrabold uppercase">WhatsApp Mobile *</label>
                                        <input type="tel" required value={formPhone} onChange={(e) => setFormPhone(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-5 py-3 text-slate-900 focus:outline-none focus:border-lub-blue text-base font-medium transition" placeholder="10 Digit Number" />
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-xs text-slate-600 font-extrabold uppercase">Industrial Sector Match *</label>
                                    <select required value={formSector} onChange={(e) => setFormSector(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-5 py-3.5 text-slate-700 focus:outline-none focus:border-lub-blue text-base font-medium transition">
                                        <option value="">Choose Industry Track</option>
                                        <option value="Automotive & Ancillary">Automotive &amp; EV Ancillaries</option>
                                        <option value="Aerospace & Defense">Aerospace &amp; Defense Corridors</option>
                                        <option value="Electricals & Automation">Electricals, Panels &amp; PLC Systems</option>
                                        <option value="Textiles & Handloom">Technical Silks &amp; Handloom Heritage</option>
                                    </select>
                                </div>
                                <div className="pt-4 flex gap-5">
                                    <button type="button" onClick={closeModal} className="flex-1 py-4 bg-slate-100 text-center rounded-xl font-extrabold text-sm uppercase tracking-wider text-slate-500 hover:bg-slate-200 transition">Dismiss</button>
                                    <button type="submit" className="flex-1 py-4 bg-gradient-to-r from-lub-blue to-lub-enterpriseMid text-center rounded-xl text-white font-extrabold text-sm uppercase tracking-wider hover:brightness-110 transition shadow-md">Proceed to Escrow</button>
                                </div>
                            </form>
                        </div>

                        <div className={`p-8 space-y-7${bookingStep === 2 ? "" : " hidden"}`}>
                            <div className="text-center space-y-2">
                                <h4 className="text-base font-extrabold text-slate-900 uppercase tracking-wider">Canara Bank Verification Routing</h4>
                                <p className="text-sm text-slate-600 font-medium">Generate UPI route pattern or copy bank account coordinates</p>
                            </div>
                            <div className="flex flex-col sm:flex-row items-center gap-6 bg-slate-50 p-6 rounded-xl border border-slate-200">
                                ...
                                <div className="flex-1 space-y-3 text-sm text-slate-700 font-semibold">
                                    <div><strong className="text-lub-blue">Account:</strong> Laghu Udyog Bharati</div>
                                    <div><strong className="text-lub-blue">Bank Name:</strong> Canara Bank</div>
                                    <div><strong className="text-lub-blue">Account No:</strong> 0908201005559</div>
                                    <div><strong className="text-lub-blue">IFSC Routing:</strong> CNRB0000936</div>
                                    <p className="text-xs text-slate-500 border-t border-slate-200 pt-2 font-medium">
                                        *Transfer exact total and input transaction reference code (UTR) below.
                                    </p>
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <label className="text-xs text-slate-600 font-extrabold uppercase">Transaction Reference (UTR) *</label>
                                    <input type="text" value={formUtr} onChange={(e) => setFormUtr(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-5 py-3.5 text-slate-900 font-mono text-base focus:outline-none focus:border-lub-blue" placeholder="e.g., CNRB1234567890" />
                                </div>
                                <button type="button" onClick={confirmPayment} className="w-full py-4 bg-gradient-to-r from-lub-green to-emerald-500 text-white font-extrabold text-base uppercase tracking-wider rounded-xl hover:brightness-110 transition shadow-lg">
                                    Submit Slip &amp; Request Hold
                                </button>
                            </div>
                        </div>

                        <div className={`p-10 text-center space-y-7${bookingStep === 3 ? "" : " hidden"}`}>
                            ...
                            <div className="space-y-3">
                                <h4 className="text-2xl font-extrabold text-slate-900 uppercase tracking-wider">Registration Logged</h4>
                                <p className="text-base text-slate-600 max-w-sm mx-auto font-medium leading-relaxed">
                                    Our regional LUB district secretary will verify the bank slip coordinates within 24 working hours to issue your credential pack.
                                </p>
                            </div>
                            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 text-left text-sm text-slate-700 space-y-3 font-mono font-semibold">
                                <div><strong>Registration Token:</strong> <span className="text-lub-orange">{successData.id}</span></div>
                                <div><strong>Selected Stall:</strong> <span className="text-lub-orange">{successData.stall}</span></div>
                                <div><strong>Registered Entity:</strong> <span className="text-slate-900">{successData.company}</span></div>
                            </div>
                            <button type="button" onClick={closeModal} className="w-full py-4 bg-gradient-to-r from-lub-blue to-lub-enterpriseMid text-white font-extrabold text-base uppercase tracking-wider rounded-xl hover:brightness-110 transition">
                ...
                {/* ===== B2B MATCHER SECTION ===== */}
                <section id="b2b-matcher" className="py-28 relative bg-lub-enterpriseBg border-t border-slate-100">
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                            ...
                            <div className="lg:col-span-5 space-y-10">
                                <div className="space-y-4">
                                    <span className="text-lub-orange uppercase font-extrabold tracking-widest text-sm">VENUE LOGISTICS</span>
                                    <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-900 leading-tight">Hotel Hills, Hosur</h2>
                                    <p className="text-lg text-slate-600 font-medium leading-relaxed">Perfect geographic placement directly off the Bengaluru-Chennai highway corridor, facilitating easy multi-modal freight integration.</p>
                                </div>
                                ...
                                <div className="rounded-2xl border border-slate-200 h-72 bg-slate-50 flex items-center justify-center text-center p-8 relative overflow-hidden">
                                    ...
                                    <div className="space-y-5 relative z-10">
                                        ...
                                        <div>
                                            <h4 className="font-extrabold text-slate-900 text-lg">Hotel Hills Exhibition Arena</h4>
                                            <p className="text-sm text-slate-600 mt-2 font-medium">Sipcot Phase 1 Corridor Adjacent, Hosur, Tamil Nadu.</p>
                                        </div>
                                        <a href="https://maps.google.com" target="_blank" className="inline-block text-sm text-lub-blue hover:underline font-bold">
                                            <i className="fa-solid fa-compass mr-2"></i> Open Google Navigation Guide
                                        </a>
                                    </div>
                                </div>

                                <div className="space-y-3 bg-slate-50 p-8 rounded-2xl border border-slate-200 text-sm text-slate-700">
                                    <h4 className="font-extrabold text-slate-900 mb-3 uppercase tracking-wider text-lub-blue text-base">LUB Tamil Nadu Secretariat Address</h4>
                                    <p className="leading-relaxed font-semibold">
                                        <strong className="text-slate-900">Laghu Udyog Bharati - Tamil Nadu State Headquarters</strong><br />
                                        Plot No 63A, First Floor, 9th Street,<br />
                            ...
                            <div className="lg:col-span-7 space-y-8">
                                <div className="space-y-4">
                                    <span className="text-lub-green uppercase font-extrabold tracking-widest text-sm">EXECUTIVE SECRETARIAT DIRECTORY</span>
                                    <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-900 leading-tight">Convenors &amp; Office Bearers</h2>
                                    <p className="text-lg text-slate-600 font-medium">Reach out directly to district and chapter convenors to complete your registration process.</p>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    ...
                                    <div className="enterprise-card p-6 rounded-xl flex items-start gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-base font-extrabold text-slate-900">Kumar Chandrasekar</h4>
                                            <p className="text-xs text-purple-500 uppercase font-extrabold font-mono">Expo Chairman</p>
                                            <a href="tel:+919840097614" className="text-sm text-lub-blue hover:underline flex items-center gap-2 font-bold mt-1">
                                    ...
                                    <div className="enterprise-card p-6 rounded-xl flex items-start gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-base font-extrabold text-slate-900">Jayendran K</h4>
                                            <p className="text-xs text-green-500 uppercase font-extrabold font-mono">Expo Vice Chairman</p>
                                            <a href="tel:+919884209989" className="text-sm text-lub-blue hover:underline flex items-center gap-2 font-bold mt-1">
                                    ...
                                    <div className="enterprise-card p-6 rounded-xl flex items-start gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-base font-extrabold text-slate-900">R. Vaidyanathan</h4>
                                            <p className="text-xs text-blue-500 uppercase font-extrabold font-mono">Convenor Expo</p>
                                            <a href="tel:+919840236534" className="text-sm text-lub-blue hover:underline flex items-center gap-2 font-bold mt-1">
                                    ...
                                    <div className="enterprise-card p-6 rounded-xl flex items-start gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-base font-extrabold text-slate-900">Senthil Kumar N</h4>
                                            <p className="text-xs text-orange-500 uppercase font-extrabold font-mono">Convenor - BSM</p>
                                            <a href="tel:+919940243428" className="text-sm text-lub-blue hover:underline flex items-center gap-2 font-bold mt-1">
                                    ...
                                    <div className="enterprise-card p-6 rounded-xl flex items-start gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-base font-extrabold text-slate-900">T.R. Manoz Kumar</h4>
                                            <p className="text-xs text-orange-500 uppercase font-extrabold font-mono">Co Convenor Expo</p>
                                            <a href="tel:+919940243428" className="text-sm text-lub-blue hover:underline flex items-center gap-2 font-bold mt-1">
                                    ...
                                    <div className="enterprise-card p-6 rounded-xl flex items-start gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-base font-extrabold text-slate-900">K. Sivakumar</h4>
                                            <p className="text-xs text-green-500 uppercase font-extrabold font-mono">Expo Event Head</p>
                                            <a href="tel:+919362310612" className="text-sm text-lub-blue hover:underline flex items-center gap-2 font-bold mt-1">
                                    ...
                                    <div className="enterprise-card p-6 rounded-xl flex items-start gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-base font-extrabold text-slate-900">Siddharth Kumaran K</h4>
                                            <p className="text-xs text-purple-500 uppercase font-extrabold font-mono">Event Coordinator</p>
                                            <a href="tel:+917550265099" className="text-sm text-lub-blue hover:underline flex items-center gap-2 font-bold mt-1">
                                    ...
                                    <div className="enterprise-card p-6 rounded-xl flex items-start gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-base font-extrabold text-slate-900">Balaji R</h4>
                                            <p className="text-xs text-red-500 uppercase font-extrabold font-mono">Treasurer</p>
                                            <a href="tel:+919600881442" className="text-sm text-lub-blue hover:underline flex items-center gap-2 font-bold mt-1">
                                    ...
                                    <div className="enterprise-card p-6 rounded-xl flex items-start gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-base font-extrabold text-slate-900">Sriram Hariharan</h4>
                                            <p className="text-xs text-amber-600 uppercase font-extrabold font-mono">Digital Support</p>
                                            <a href="tel:+919840727309" className="text-sm text-lub-blue hover:underline flex items-center gap-2 font-bold mt-1">
                ...
                <footer className="bg-lub-enterpriseDark py-16 border-t border-slate-700 text-slate-300 space-y-10">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-8 text-sm uppercase tracking-wider font-bold">
                        ...
                        <a href="#sectors" onClick={(e) => handleSmoothScroll(e, "#sectors")} className="hover:text-white transition">Focus Sectors</a>
                        <a href="#heritage" onClick={(e) => handleSmoothScroll(e, "#heritage")} className="hover:text-white transition">Heritage Hub</a>
                        <a href="#supported-sponsored" onClick={(e) => handleSmoothScroll(e, "#supported-sponsored")} className="hover:text-white transition">Supported &amp; Sponsored By</a>
                        <a href="#floorplan" onClick={(e) => {
                            handleSmoothScroll(e, "#brochure")
                            switchBrochureTab("p2")
                        }}
                            className="hover:text-white transition">Live Floorplan</a>
                    </div>

                    {/* Logos */}
                    <div className="flex items-center justify-center gap-6 py-2">
                        <img src={ASSETS.lubLogo} alt="LUB Logo" className="h-16 w-auto object-contain opacity-80 hover:opacity-100 transition" />
                        <img src={ASSETS.msmeSangamam} alt="MSME Sangamam" className="h-16 w-auto object-contain opacity-80 hover:opacity-100 transition" />
                    </div>

                    {/* Disclaimer */}
                    <p className="max-w-lg mx-auto text-xs sm:text-sm leading-relaxed text-slate-400 font-medium text-center">
                        Disclaimer: This portal works as the official interactive digital interface of Laghu Udyog Bharati's MSME Sangamam Connect Expo, scheduled on Sep 18 &amp; 19, 2026, at Hotel Hills, Hosur, Tamil Nadu.
                    </p>
                    ...
                    <div className="pt-6 border-t border-slate-700/80 max-w-2xl mx-auto flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-slate-100 text-xs sm:text-sm font-bold px-4 text-center">
                        <span className="font-bold text-slate-100">&copy; 2026 Laghu Udyog Bharati - Tamil Nadu State. All Rights Reserved.</span>
                        <span className="hidden sm:inline text-slate-400 font-bold">&bull;</span>
